...
def student_register(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            # Save the form data to create a new student user
            user = form.save(commit=False)
            user.user_type = 'student'  # Set the user type
            user.save()

            # You can add additional logic like sending a welcome email or redirect to a profile page
...
def company_register(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            # Save the form data to create a new company user
            user = form.save(commit=False)
            user.user_type = 'company'  # Set the user type
            user.save()
